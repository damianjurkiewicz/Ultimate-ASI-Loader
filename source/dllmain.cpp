...
bool EndsWithCaseInsensitive(std::wstring_view value, std::wstring_view suffix)
{
    ...
    auto valueIt = value.end() - suffix.length();
    return std::equal(valueIt, value.end(), suffix.begin(), suffix.end(), [](wchar_t a, wchar_t b)
    {
        return ::towlower(a) == ::towlower(b);
    });
}

void AppendLoadingOrderFromFile(const std::wstring& file, std::unordered_map<std::wstring, int>& loadingOrder)
{
...
std::unordered_map<std::wstring, int> ParseLoadingOrder(const std::vector<std::wstring>& fileNames)
{
    ...
    auto scriptIni = std::find_if(fileNames.begin(), fileNames.end(), [](const std::wstring& path)
    {
        return EndsWithCaseInsensitive(path, L"scripts\\global.ini") || EndsWithCaseInsensitive(path, L"scripts/global.ini");
    });

    if (scriptIni != fileNames.end())
    {
        AppendLoadingOrderFromFile(*scriptIni, loadingOrder);
        if (!loadingOrder.empty())
            return loadingOrder;
    }

    for (const auto& file : fileNames)
    {
        if (scriptIni != fileNames.end() && &file == &(*scriptIni))
            continue;

        AppendLoadingOrderFromFile(file, loadingOrder);
    }
